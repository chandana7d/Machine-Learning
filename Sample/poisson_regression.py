...
def report_function(e,params,X,Y,offset,X_val,Y_val,offset_val):
    loss=PoissonError(params,X,Y,offset)
    l=poisson_lambda(params,X,offset)
    err=(l-Y)**2/l
    train_accuracy=np.mean(err)
    count_err=count_error(np.round(l),Y)
    print(f"\t{e}: Train Loss {loss:.4f}, Disp {train_accuracy:.4f}, Err {count_err:.4f}",end=",")
    if not(X_val is None):
        val_loss=PoissonError(params,X_val,Y_val,offset_val)
        l=poisson_lambda(params,X_val,offset_val)
        err=(l-Y_val)**2/l
        val_accuracy=np.mean(err)
        val_err=count_error(np.round(l),Y_val)
        print(f"Val Loss {val_loss:.4f}, Disp {val_accuracy:.4f}, Err {val_err:.4f}")
    else:
        print()
